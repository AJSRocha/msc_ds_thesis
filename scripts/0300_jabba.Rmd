...
load(df, frota_cerco, file = 'data/initial_data_occ_df_limpo_ps.Rdata')
```
...
```{r}
jbinput_w = build_jabba(catch = data.frame('Year' = df_y_MIS_W$year_sale %>% as.numeric(),
                                         'mis' = df_y_MIS_W$catch,
                                         'otb' = df_y_OTB_W$catch),
                      cpue = data.frame('Year' = df_y_MIS_W$year_sale %>% as.numeric(),
                                        'mis' = df_y_MIS_W$lpue_y,
                                        'otb' = df_y_OTB_W$lpue_y),
                      se = data.frame('Year' = df_y_MIS_W$year_sale %>% as.numeric(),
                                        'mis' = log(df_y_MIS_W$sd/df_y_MIS_W$lpue_y),
                                        'otb' = log(df_y_OTB_W$sd/df_y_OTB_W$lpue_y)),
                      scenario = 'TestRun',
                      model.type = 'Fox',
                      sigma.est = F,
                      fixed.obsE = 0.01)
...
xtable(bet_total_w$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_est_w.tex", 
        include.rownames = T,
        only.contents = T)
```

```{r, fig.height=10, fig.width=10}
jbplot_ppdist(bet_total_w, mfrow = c(2,4))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm', filename = 'plots/resjabba_pardist_w.png')
```
...
```{r}
jbinput_s = build_jabba(catch = data.frame('Year' = df_y_MIS_S$year_sale %>% as.numeric(),
                                         'mis' = df_y_MIS_S$QVENDA,
                                         'otb' = df_y_OTB_S$QVENDA),
                      cpue = data.frame('Year' = df_y_MIS_S$year_sale %>% as.numeric(),
                                        'mis' = df_y_MIS_S$QVENDA/df_y_MIS_S$effort,
                                        'otb' = df_y_OTB_S$QVENDA/df_y_OTB_S$effort),
                      scenario = 'TestRun',
                      model.type = 'Fox',
                      sigma.est = F,
                      fixed.obsE = 0.01)
...
xtable(bet_total_s$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_est_s.tex", 
        include.rownames = T,
        only.contents = T)
```

```{r, fig.height=10, fig.width=10}
jbplot_ppdist(bet_total_s, mfrow = c(2,4))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm', filename = 'plots/resjabba_pardist_s.png')
```

```{r, fig.height=10, fig.width=10}

# dev.new()
png('plots/res_jabba_s.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
...
```{r, fig.height=10, fig.width=10}
jbplot_ppdist(bet_total, mfrow = c(2,4))
