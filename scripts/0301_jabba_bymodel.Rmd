...
#Schaefer
a = Sys.time()
input = df_effort_y %>% filter(fill == '1-Western Coast 1-Polyvalent')

jbinput_w_schaefer = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = log(input$sd/input$lpue)),
                      scenario = 'TestRun',
                      model.type = 'Schaefer',
                      sigma.est = F,
                      fixed.obsE = 0.01,
                      Plim = 0.25)
b = Sys.time()
perf_jabba_schaef_preproc_mis_w = b-a

#Pella
a = Sys.time()
input = df_effort_y %>% filter(fill == '1-Western Coast 1-Polyvalent')

jbinput_w_pella = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
                      ...
                      Plim = 0.25)
b = Sys.time()
perf_jabba_pella_preproc_mis_w = b-a

#Fox
a = Sys.time()
input = df_effort_y %>% filter(fill == '1-Western Coast 1-Polyvalent')

jbinput_w_fox = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
...
b = Sys.time()
perf_jabba_fox_preproc_mis_w = b-a
...
#Schaefer
a = Sys.time()
bet_total_w_schaefer = fit_jabba(jbinput_w_schaefer,
                    ...
                    verbose = TRUE)
b = Sys.time()
perf_jabba_schaefer_fit_mis_w = b-a

#Pella
a = Sys.time()
bet_total_w_pella = fit_jabba(jbinput_w_pella,
                    ni = 30000, #number of iterations 
                    nt = 5, # thinning interval of saved iterations
                    nb = 5000, #burn-in
                    nc = 2, #number of mcmc chains initial values
                    ...
                    verbose = TRUE)
b = Sys.time()
perf_jabba_pella_fit_mis_w = b-a

#Fox
a = Sys.time()
bet_total_w_fox = fit_jabba(jbinput_w_fox,
                    ni = 30000, #number of iterations 
                    nt = 5, # thinning interval of saved iterations
                    nb = 5000, #burn-in
                    nc = 2, #number of mcmc chains initial values
...
b = Sys.time()
perf_jabba_fox_fit_mis_w = b-a



jabba_table_3 =
  rbind(
  bet_total_w_schaefer$estimates %>% mutate(
    var = row.names(.),
    model = 'Schaefer') %>% 
    dplyr::select(var, model, mu, lci, uci),
  bet_total_w_pella$estimates %>% mutate(var = row.names(.),
                                         model = 'Pella-Tomlinson') %>% 
    dplyr::select(var, model, mu, lci, uci),
  bet_total_w_fox$estimates %>% mutate(model = 'Fox',
                                       var = row.names(.),) %>% 
    dplyr::select(var, model, mu, lci, uci)) %>%
    arrange(factor(var, levels = row.names(bet_total_w_fox$estimates)))



xtable(jabba_table_3) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_all_models_est_mis_w.tex", 
        include.rownames = F,
        only.contents = T)
...
## schaefer
a = Sys.time()
xtable(bet_total_w_schaefer$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_schaefer_est_mis_w.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_w_schaefer, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_schaefer_pardist_w.png')

# dev.new()
png('plots/res_jabba_schaefer_w.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_w_schaefer,type="BBmsy", add = T)
jbplot_trj(bet_total_w_schaefer,type="FFmsy", add = T)
jbplot_kobe(bet_total_w_schaefer, add = T)
jbplot_spphase(bet_total_w_schaefer, add = T)
dev.off()

b = Sys.time()
perf_jabba_schaefer_render_mis_w = b-a

## Pella 

a = Sys.time()
xtable(bet_total_w_pella$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_pella_est_mis_w.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_w_pella, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_pella_pardist_w.png')

# dev.new()
png('plots/res_jabba_pella_w.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_w_pella,type="BBmsy", add = T)
jbplot_trj(bet_total_w_pella,type="FFmsy", add = T)
jbplot_kobe(bet_total_w_pella, add = T)
jbplot_spphase(bet_total_w_pella, add = T)
dev.off()

b = Sys.time()
perf_jabba_pella_render_mis_w = b-a

## Fox 

a = Sys.time()
xtable(bet_total_w_fox$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_fox_est_mis_w.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_w_fox, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_fox_pardist_w.png')

# dev.new()
png('plots/res_jabba_fox_w.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_w_fox,type="BBmsy", add = T)
jbplot_trj(bet_total_w_fox,type="FFmsy", add = T)
jbplot_kobe(bet_total_w_fox, add = T)
jbplot_spphase(bet_total_w_fox, add = T)
dev.off()

b = Sys.time()
perf_jabba_fox_render_mis_w = b-a
```
...
#Schaefer
a = Sys.time()
input = df_effort_y %>% filter(fill == '2-Southern Coast 1-Polyvalent')

jbinput_s_schaefer = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
                      ...
                      Plim = 0.25)
b = Sys.time()
perf_jabba_schaef_preproc_mis_s = b-a

#Pella
a = Sys.time()
input = df_effort_y %>% filter(fill == '2-Southern Coast 1-Polyvalent')

jbinput_s_pella = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
                      ...
                      Plim = 0.25)
b = Sys.time()
perf_jabba_pella_preproc_mis_s = b-a

#Fox
a = Sys.time()
input = df_effort_y %>% filter(fill == '2-Southern Coast 1-Polyvalent')

jbinput_s_fox = build_jabba(catch = data.frame('Year' = input$year_sale %>% as.numeric(),
                                         'mis' = input$catch),
                      cpue = data.frame('Year' = input$year_sale %>% as.numeric(),
                                        'mis' = input$lpue),
                      se = data.frame('Year' = input$year_sale %>% as.numeric(),
...
b = Sys.time()
perf_jabba_fox_preproc_mis_s = b-a


#Schaefer
a = Sys.time()
bet_total_s_schaefer = fit_jabba(jbinput_s_schaefer,
                    ni = 30000, #number of iterations 
                    nt = 5, # thinning interval of saved iterations
                    nb = 5000, #burn-in
                    nc = 2, #number of mcmc chains initial values
                    ...
                    verbose = TRUE)
b = Sys.time()
perf_jabba_schaefer_fit_mis_s = b-a

#Pella
a = Sys.time()
bet_total_s_pella = fit_jabba(jbinput_s_pella,
                    ni = 30000, #number of iterations 
                    nt = 5, # thinning interval of saved iterations
                    nb = 5000, #burn-in
                    nc = 2, #number of mcmc chains initial values
                    ...
                    verbose = TRUE)
b = Sys.time()
perf_jabba_pella_fit_mis_s = b-a

#Fox
a = Sys.time()
bet_total_s_fox = fit_jabba(jbinput_s_fox,
                    ni = 30000, #number of iterations 
                    nt = 5, # thinning interval of saved iterations
                    nb = 5000, #burn-in
                    nc = 2, #number of mcmc chains initial values
...
b = Sys.time()
perf_jabba_fox_fit_mis_s = b-a



jabba_table_3 =
  rbind(
  bet_total_s_schaefer$estimates %>% mutate(
    var = row.names(.),
    model = 'Schaefer') %>% 
    dplyr::select(var, model, mu, lci, uci),
  bet_total_s_pella$estimates %>% mutate(var = row.names(.),
                                         model = 'Pella-Tomlinson') %>% 
    dplyr::select(var, model, mu, lci, uci),
  bet_total_s_fox$estimates %>% mutate(model = 'Fox',
                                       var = row.names(.),) %>% 
    dplyr::select(var, model, mu, lci, uci)) %>%
    arrange(factor(var, levels = row.names(bet_total_s_fox$estimates)))
...
xtable(jabba_table_3) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_all_models_est_mis_s.tex", 
        include.rownames = F,
        only.contents = T)
...
## schaefer
a = Sys.time()
xtable(bet_total_s_schaefer$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_schaefer_est_mis_s.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_s_schaefer, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_schaefer_pardist_s.png')

# dev.new()
png('plots/res_jabba_schaefer_s.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_s_schaefer,type="BBmsy", add = T)
jbplot_trj(bet_total_s_schaefer,type="FFmsy", add = T)
jbplot_kobe(bet_total_s_schaefer, add = T)
jbplot_spphase(bet_total_s_schaefer, add = T)
dev.off()

b = Sys.time()
perf_jabba_schaefer_render_mis_s = b-a

## Pella 

a = Sys.time()
xtable(bet_total_s_pella$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_pella_est_mis_s.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_s_pella, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_pella_pardist_s.png')

# dev.new()
png('plots/res_jabba_pella_s.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_s_pella,type="BBmsy", add = T)
jbplot_trj(bet_total_s_pella,type="FFmsy", add = T)
jbplot_kobe(bet_total_s_pella, add = T)
jbplot_spphase(bet_total_s_pella, add = T)
dev.off()

b = Sys.time()
perf_jabba_pella_render_mis_s = b-a

## Fox 

a = Sys.time()
xtable(bet_total_s_fox$estimates) %>% 
  print(.,
        type = "latex",
        file = "tables/res_jabba_fox_est_mis_s.tex", 
        include.rownames = T,
        only.contents = T)

jbplot_ppdist(bet_total_s_fox, mfrow = c(2,3))

grid.echo()
fig = grid.grab()
ggsave(fig, dpi = 300, units = 'cm',
       height = 10, width = 10, filename = 'plots/res_jabba_fox_pardist_s.png')

# dev.new()
png('plots/res_jabba_fox_s.png', height = 10, width = 10, units = 'in', res = 300)
par(mfrow = c(2,2), mar = c(5, 5, 5, 5))
jbplot_trj(bet_total_s_fox,type="BBmsy", add = T)
jbplot_trj(bet_total_s_fox,type="FFmsy", add = T)
jbplot_kobe(bet_total_s_fox, add = T)
jbplot_spphase(bet_total_s_fox, add = T)
dev.off()

b = Sys.time()
perf_jabba_fox_render_mis_s = b-a
```
...
perf_jabba_all = data.frame('Model' = rep(c('Schaefer', 'Pella-Tomlinson', 'Fox'),2),
  'Framework' = c(rep('JABBA - Polyvalent, Western Coast, Yearly data',3),
                  rep('JABBA - Polyvalent, Southern Coast, Yearly data',3)),
  'Pre-process' = c(perf_jabba_schaef_preproc_mis_w,
                    perf_jabba_pella_preproc_mis_w,
                    perf_jabba_fox_preproc_mis_w,
                    perf_jabba_schaef_preproc_mis_s,
                    perf_jabba_pella_preproc_mis_s,
                    perf_jabba_fox_preproc_mis_s),
  'Model fit' = c(perf_jabba_schaefer_fit_mis_w,
                  perf_jabba_pella_fit_mis_w,
                  perf_jabba_fox_fit_mis_w,
                  perf_jabba_schaefer_fit_mis_s,
                  perf_jabba_pella_fit_mis_s,
                  perf_jabba_fox_fit_mis_s),
  'Output Render' = c(perf_jabba_schaefer_render_mis_w,
                      perf_jabba_pella_render_mis_w,
                      perf_jabba_fox_render_mis_w,
                      perf_jabba_schaefer_render_mis_s,
                      perf_jabba_pella_render_mis_s,
                      perf_jabba_fox_render_mis_s))

save(perf_jabba_all, file = 'benchmarks/jabba_all.Rdata')
save(bet_total_s_schaefer,
     bet_total_s_pella,
     bet_total_s_fox,
     bet_total_w_schaefer,
     bet_total_w_pella,
     bet_total_s_fox,  file = 'tables/jabba_all_tables.Rdata')
```
