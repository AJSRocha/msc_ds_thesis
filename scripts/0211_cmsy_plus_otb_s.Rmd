...
Usando o ficheiro do algarve_mis_mis que sobrou do spict, criar os csv necssarios para o CSMY++
Alguns priors foram tirandos de https://www.sealifebase.se/summary/Octopus-vulgaris.html
...
# Continent,Region,Subregion,Stock,Group,Name,ScientificName,SpecCode,Source,MinOfYear,MaxOfYear,StartYear,EndYear,Flim,Fpa,Blim,Bpa,Bmsy,MSYBtrigger,Fmsy,last_F,Resilience,r.low,r.hi,stb.low,stb.hi,int.yr,intb.low,intb.hi,endb.low,endb.hi,btype,e.creep,force.cmsy,Comments,



id = data.frame(Continent = 'Europe',
                Region = 'Iberia',
                Subregion = 'Portugal',
                Stock = "2-Southern Coast 2-Bottom Trawl",
                Group = 'Cephalopoda',
                Name = 'Octopus',
...
catch = data.frame(Stock = df_effort_y$fill,
...
nn_file     =  "scripts/CMSY++/ffnn.bin" # file containing neural networks trained to estimate B/k priors
outfile     = paste("Out_",format(Sys.Date(),format="%B%d%Y_"),'text_goes_here',sep="") # default name for output file

stocks = "2-Southern Coast 2-Bottom Trawl"
...
# Parametros iniciais e funçoes necessarias a analise
...
retros       <- T # set to TRUE to enable retrospective analysis (1-3 years less in the time series)
...
# cat("Files", catch, ",", id,",",nn_file,"read successfully","\n")
...
for(stock in stocks) {
  ...
  # assign data from cinfo to vectors
    ...
    if(mean.ct_MSY.start >= ct_MSY.lim) {
      startbio    <- bk.MSY
        ...
        pr.nn.startbio <- neuralnet::compute(nn.startbio, nninput.start)
    ...
    pr.nn.intbio   <- neuralnet::compute(nn.intbio, nninput.int)
    ...
    # estimate endbio
      ...
      if(mean.ct_MSY.end >= ct_MSY.lim) {
        endbio    <- bk.MSY
         ...
         pr.nn.endbio   <- neuralnet::compute(nn.endbio, nninput.end)
 ...
 #   bt           <- bt.raw
  ...
  time_f = Sys.time()
  perf_cmsy_fit_otb_s = time_f - time_i
...
time_f = Sys.time()
perf_cmsy_render_otb_s = time_f - time_i
check = time_f - total_i

```

```{r}
perf_cmsy_s_otb = data.frame('Framework' = 'CMSY++ - Bottowm Trawl, Southern Coast, Yearly data',
                              'Pre-process' = perf_cmsy_preproc_otb_s,
                              'Model fit' = perf_cmsy_fit_otb_s,
                              'Output Render' = check-perf_cmsy_fit_otb_s)
save(perf_cmsy_s_otb, file = 'benchmarks/cmsy_otb_s_y.Rdata')
```
