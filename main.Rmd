...
# Esforço nominal - desistimos do resto

```{r}
df_effort_y =
  df_int %>%
  # mutate(predicoes = pred) %>% 
  group_by(year_sale, EGRUPART, regiao) %>% 
  summarise(catch = sum(QVENDA),
            lpue = mean(QVENDA),
            effort = length(QVENDA),
            sd = sd(QVENDA)) %>%
  mutate(regiao = case_when(regiao == 'Costa Ocidental' ~ '1-Western Coast',
                            T ~ '2-Southern Coast'),
         EGRUPART = case_when(EGRUPART == 'MIS_MIS' ~ '1-Polyvalent',
...
df_effort_m =
  df_int %>%
  # mutate(predicoes = pred) %>% 
  group_by(year_sale, month_sale, EGRUPART, regiao) %>% 
  summarise(catch = sum(QVENDA),
            lpue = mean(QVENDA),
            effort = length(QVENDA),
            sd = sd(QVENDA)) %>%
...
df_effort_w =
  df_int %>%
  # mutate(predicoes = pred) %>% 
  group_by(year_sale, semana, EGRUPART, regiao) %>% 
  summarise(catch = sum(QVENDA),
            lpue = mean(QVENDA),
            effort = length(QVENDA),
            sd = sd(QVENDA)) %>%
...
save(df_int, df_effort_y, df_effort_m, df_effort_w, file = 'data/initial_data_occ_df_final.Rdata')
```

# ESFORÇO

* Começar com esforço nominal - 1 viagem

* LPUE = lg/viagem

* Ponderar esforço do arrasto com metodo do Hugo

* Criar indice alternativo estandardizado com tweedie

# Pondera esforço semanal e anual com metodologia do hugo

```{r}
# IEP_y =
# df_int %>% 
#   mutate(esforco = 1) %>% 
#   group_by(year_sale, EGRUPART) %>% 
#   summarise(C_i = sum(QVENDA),
#             N_i = sum(esforco)) %>% 
#   mutate(e_i = C_i/N_i)
# 
# E_arte_y = IEP_y %>% group_by(EGRUPART) %>% 
#   summarise(e_arte = sum(e_i)/28)
# 
# E_arte_y = E_arte_y %>%
#   mutate(e_arte_r = e_arte/sum(e_arte))
# 
# IEP_y = IEP_y %>%
#   mutate(
#     e_i_r = case_when(EGRUPART == 'OTB' ~ N_i * E_arte_y$e_arte_r[2],
#                       T ~ N_i * E_arte_y$e_arte_r[1]),
#     check = e_i_r/N_i)
# 
# 
# IEP_w =
# df_int %>% 
#   mutate(esforco = 1) %>% 
#   group_by(year_sale, semana, EGRUPART) %>% 
#   summarise(C_i = sum(QVENDA),
#             N_i = sum(esforco)) %>% 
#   mutate(e_i = C_i/N_i)
# 
# E_arte_w = IEP_w %>% group_by(EGRUPART) %>% 
#   summarise(e_arte = sum(e_i)/28)
# 
# E_arte_w = E_arte_w %>%
#   mutate(e_arte_r = e_arte/sum(e_arte))
...
# IEP_w = IEP_w %>%
#   mutate(
#     e_i_r = case_when(EGRUPART == 'OTB' ~ N_i * E_arte_w$e_arte_r[2],
#                       T ~ N_i * E_arte_w$e_arte_r[1]),
#     check = e_i_r/N_i)
```

# Standardiza com Tweedie

```{r}
a = Sys.time()
df_int = 
df_int %>% group_by(EGRUPART, year_sale, regiao) %>%
mutate(Power.Main=ifelse(is.na(Power.Main),median(Power.Main,na.rm=TRUE),Power.Main))
...
out_mis_w = tweedie.profile(QVENDA ~ year_sale + factor(semana) +  Power.Main + QVENDA_total,
                      p.vec=seq(1.01, 1.9, length=9), link.power=0,
                      data=df_int_mis_w,
                      method="interpolation",
                      do.ci=TRUE, do.smooth=TRUE, do.plot=TRUE, phi.method="saddlepoint")

out_mis_s = tweedie.profile(QVENDA ~ year_sale + factor(semana) +  Power.Main + QVENDA_total,
                      p.vec=seq(1.01, 1.9, length=9), link.power=0,
                      data=df_int_mis_s,
                      method="interpolation",
                      do.ci=TRUE, do.smooth=TRUE, do.plot=TRUE, phi.method="saddlepoint")
...
# 
# a = Sys.time()
std_mis_w = glm(QVENDA ~  year_sale + factor(semana) +  Power.Main + QVENDA_total,
          data = df_int_mis_w, family = tweedie(var.power = out_mis_w$p.max, link.power = 0))

std_mis_s = glm(QVENDA ~  year_sale + factor(semana) +  Power.Main + QVENDA_total,
          data = df_int_mis_s, family = tweedie(var.power = out_mis_s$p.max, link.power = 0))
# b = Sys.time() #59.48 segundos
# b-a


ls.Model_y = lsmeans(std, c("year_sale","EGRUPART","regiao"))
ls.Model_w = lsmeans(std, c('year_sale',"EGRUPART","regiao", 'semana'))

ls.Model_y = data.frame(ls.Model_y)
ls.Model_w = data.frame(ls.Model_w)
...
b = Sys.time() 
b-a # Time difference of 36.02784 mins
```
...
df_effort_y =
  ...
  group_by(year_sale, EGRUPART, regiao) %>% 
  summarise(catch = sum(QVENDA),
            lpue_nom = mean(QVENDA),
            lpue_2 = mean(pred),
            effort = length(QVENDA),
            sd_nom = sd(QVENDA)) %>% 
  left_join(.,
            ls.Model_y,
            by = c('EGRUPART', 'year_sale','regiao')) %>% 
  mutate(lpue_std = scale(exp(lsmean),center = F,scale=T),
         sd_std = scale(exp(SE),center = F,scale=T),
         asymp.LCL = scale(exp(asymp.LCL), center = F,scale=T),
         asymp.UCL = scale(exp(asymp.UCL), center = F,scale=T)) %>% 
  mutate(regiao = case_when(regiao == 'Costa Ocidental' ~ '1-Western Coast',
                            T ~ '2-Southern Coast'),
         EGRUPART = case_when(EGRUPART == 'MIS_MIS' ~ '1-Polyvalent',
                            T ~ '2-Bottom Trawl'),
...
df_effort_y %>% 
  ggplot() + 
  geom_line(aes(x = year_sale, y = lpue_nom, group = fill, col = fill)) + 
  theme(legend.position = 'bottom')

df_effort_y %>% 
  ggplot() + 
  geom_line(aes(x = year_sale, y = lsmean, group = fill, col = fill)) + 
  theme(legend.position = 'bottom')

df_effort_y %>% 
  ggplot() + 
  geom_line(aes(x = year_sale, y = lpue_2, group = fill, col = fill)) + 
  theme(legend.position = 'bottom')
```
...
IEP_y %>% 
  ggplot() + 
  geom_line(aes(x = year_sale, y = C_i/N_i, group = EGRUPART, color = EGRUPART)),

IEP_y %>% 
  ggplot() + 
  geom_line(aes(x = year_sale, y = C_i/e_i_r, group = EGRUPART, color = EGRUPART)) + 
  scale_color_manual(values = c('purple','green')),
ncol=1)

grid.arrange(
IEP_w %>% 
  mutate(x = paste(year_sale, semana)) %>% 
  ggplot() + 
  geom_line(aes(x = x, y = C_i/N_i, group = EGRUPART, color = EGRUPART)),

IEP_w %>% 
  mutate(x = paste(year_sale, semana)) %>% 
  ggplot() + 

  geom_line(aes(x = x, y = C_i/e_i_r, group = EGRUPART, color = EGRUPART)) + 
  scale_color_manual(values = c('purple','green')),
ncol=1)
```
