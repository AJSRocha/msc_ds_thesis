...
```{r}
library(forecast)
#library(ggfortify); library(gridExtra); #library(ggplotify)
library(tidyverse)
library(wesanderson)
...
ts_w_S = df %>%
  ...
  ungroup() %>% 
  select(QVENDA) %>% 
  ts(start = 1995, end= 2022, frequency = 52)
...
ts_m_S = df %>%
  ...
  ungroup() %>% 
  select(QVENDA) %>% 
  ts(start = 1995, end= 2022, frequency = 12)
...
ts_q_S = df %>%
  ...
  ungroup() %>% 
  select(QVENDA) %>% 
  ts(start = 1995, end= 2022, frequency = 4)
...
ts_y_S = df %>%
  ...
  ungroup() %>% 
  select(QVENDA) %>% 
  ts(start = 1995, end= 2022, frequency = 1)
...
Proposta: Freqência mensal. Reflecte bem sazonalidade e reduz variabilidade intramensal.
...
```{r}
grid.arrange(
autoplot(ts_m_W %>% diff) + theme_classic() + labs(title = 'W - Mensal'),
autoplot(ts_m_S %>% diff) + theme_classic() + labs(title = 'S - Mensal'),
ncol = 1)
```
...
```{r}
grid.arrange(
autoplot(ts_m_S) + theme_classic() + labs(title = 'S - Mensal'),
autoplot(ts_m_S %>% diff) + theme_classic() + labs(title = 'S - Mensal (diff(1)'),
diff(ts_m_S, 1) %>% ggAcf(lag.max = 60),
diff(ts_m_S, 12) %>% ggAcf(lag.max = 60),
ncol=2)
```
...
```{r}
BoxCox.lambda(ts_m_S)
tseries::adf.test(ts_m_S, alternative='s') # pvalue = 0.01
tseries::kpss.test(ts_m_S)
```

Necesssidade de diferenciar? Aparentemente não

```{r}
nsdiffs(ts_m_S)
ndiffs(diff(ts_m_S,12))
```
#### Modelacao

```{r}
dts.train = window(ts_m_S, end=c(2020,12))
dts.test=window(ts_m_S, start=c(2021,1), end=c(2021,12))
...
```{r}
model1=astsa::sarima(dts.train,1,0,1,0,1,0,12); model1

m1 = Arima(dts.train, order = c(2,0,2), seasonal=list(order=c(0,0,2), period=12))
m1.f = forecast(m1,h=12)
...
gridExtra::grid.arrange(
  autoplot(m1.f,
         alpha = 1,
         size = 0.5) +
    labs(title = expression('SARIMA(2,0,2)*(0,0,2)'[12])) +
    theme_light(),
  autoplot(dts.train) +
